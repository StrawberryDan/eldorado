...
impl HeightMap {
    ...
    pub fn new(width: usize, height: usize) -> Self {
        HeightMap{ width, height, data: vec![0; width * height] }
    }
    ...
    pub fn neighbours(&self, x: usize, y: usize) -> Vec<([usize; 2], u16)> {
        return self.orthoganal_neighbours(x, y).into_iter().chain(self.diagonal_neighbours(x, y).into_iter()).collect();
    }
...
impl From<Image> for HeightMap {
    fn from(image: Image) -> Self {
        Self { width: image.width(), height: image.height(), data: image.data().iter().map(|c| (c[0] * u16::MAX as f64).round() as u16).collect() }
    }
}
